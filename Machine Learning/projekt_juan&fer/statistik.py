...
def playagainsteachother(field):
    print("playagainsteachother")
    field = minmaxingXAlphaPruning(field)
    field = minmaxingOAlphaPruning(field)
    field = minmaxingXAlphaPruning(field)
    field = minmaxingOAlphaPruning(field)
    field = minmaxingXAlphaPruning(field)
    field = minmaxingOAlphaPruning(field)
...
def minmaxingXAlphaPruning(field):
...
def minmaxingOAlphaPruning(field):
                ...
                for j in i:
                    if arcade.check_for_collision(pseudosprite, j) and j.owner == None:
                        ...
                        if check_for_victory(self.gitter):
                            ...
                            if self.bot != "d":
                                if self.bot == "a":
                                    self.gitter = minmaxingX(self.gitter)
                                elif self.bot == "b":
                                    self.gitter = minmaxingXAlphaPruning(self.gitter)
                                elif self.bot == "c":
                                    self.gitter = playagainsteachother(self.gitter)
                                a = self.players[0]
                                self.players.pop(0)
                                self.players.append(a)
                                self.activeplayer = self.players[0]
